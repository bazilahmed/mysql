...
SELECT s.first_name, s.last_name, SUM(p.amount) AS total_amount_rung FROM staff s JOIN payment p
ON s.staff_id = p.staff_id
GROUP BY s.first_name, s.last_name;

-- * 6c. List each film and the number of actors who are listed for that film. Use tables `film_actor` and `film`. Use inner join.
SELECT f.title, COUNT(a.actor_id) AS number_of_actors FROM film f INNER JOIN film_actor a 
ON f.film_id = a.film_id
GROUP BY f.title;

-- * 6d. How many copies of the film `Hunchback Impossible` exist in the inventory system?
SELECT f.title, COUNT(i.film_id) AS copies FROM film f 
...
SELECT s.store_id, SUM(p.amount) AS business FROM payment p 
...
CREATE VIEW top_grossing_genres_by_revenue AS 
SELECT name, SUM(p.amount) AS gross_revenue FROM category c
JOIN film_category fc 
ON c.category_id = fc.category_id
JOIN inventory i 
...
SELECT * FROM top_grossing_genres_by_revenue;

-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW top_grossing_genres_by_revenue;
